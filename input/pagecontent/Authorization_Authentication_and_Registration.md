...
<p>§c4dic-035 Client applications and Payers SHALL support the standalone launch sequence (launch/Patient) of the <a href="http://hl7.org/fhir/smart-app-launch/app-launch.html">SMART App Launch framework</a> for user authorization and client authentication. § §c4dic-036 Payers SHALL publish their authorization and token endpoints for discovery in accordance with the SMART App Launch framework. § §c4dic-037 If the app needs to authenticate the identity of the end-user, it MAY include two OpenID Connect scopes: openid and fhirUser. § §c4dic-038 When these scopes are requested, and the request is granted, the app MAY receive an id token along with the access token. §</p>
